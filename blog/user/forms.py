...
class RegistrationForm(FlaskForm):
    username = StringField(_l('Username'), validators=[
                           DataRequired(), InputRequired()])
    full_name = StringField(_l('Full name (optional)'),
                            validators=[Length(min=0, max=128)])
    email = StringField(_l('Email'), validators=[
                        DataRequired(), InputRequired(), Email()])
    telegram = StringField(_l('Username Telegram (optional)'), validators=[
    ...
    password2 = PasswordField(_l('Confirm password'), validators=[
                              DataRequired(), InputRequired(), EqualTo('password')])
    about_me = TextAreaField(_l('About me (optional)'), validators=[
                             Length(min=0, max=256), Optional()])
...
class EditProfilForm(FlaskForm):
    userid = StringField(_l('ID user'), validators=[
                         DataRequired(), InputRequired()])
    username = StringField(_l('Username'), validators=[
                           DataRequired(), InputRequired()])
    full_name = StringField(_l('Full name (optional)'), validators=[
                            Length(min=0, max=128), Optional()])
    email = StringField(_l('Email'), validators=[
                        DataRequired(), InputRequired(), Email()])
    telegram = StringField(_l('Username Telegram (optional)'), validators=[
