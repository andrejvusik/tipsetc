...
class UserModelCase(unittest.TestCase):
    ...
    def test_follow_posts(self):
        ...
        # create four posts
        now = datetime.utcnow()
        p1 = Posts(content="post from john", author=u1,
                  timestamp=now + timedelta(seconds=1))
        p2 = Posts(content="post from susan", author=u2,
                  timestamp=now + timedelta(seconds=4))
        p3 = Posts(content="post from mary", author=u3,
                  timestamp=now + timedelta(seconds=3))
        p4 = Posts(content="post from david", author=u4,
        ...
        f1 = u1.followed_and_their_posts().all()
        f2 = u2.followed_and_their_posts().all()
        f3 = u3.followed_and_their_posts().all()
        f4 = u4.followed_and_their_posts().all()
